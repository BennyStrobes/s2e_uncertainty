...
vgt_to_bs_borzoi_info = {}
for tissue_name in ordered_tissues:
	filer = organized_borzoi_gtex_predictions + tissue_name + '_random_rc_borzoi_estimates_w_uncertainty.txt'
	f = open(filer)
	head_count = 0
	for line in f:
		line = line.rstrip()
...
f = open(processed_fm_eqtl_output_file)
t = open(organized_borzoi_gtex_predictions + 'cross_bootstrap_PIP_0.9_random_rc_borzoi_pred_eqtl_effects_cross_tissue.txt','w')
head_count = 0
for line in f:
	line = line.rstrip()
	data = line.split('\t')
