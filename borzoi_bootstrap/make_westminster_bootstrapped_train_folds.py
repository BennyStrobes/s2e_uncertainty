...
def main():
	...
	rep_options.add_option(
		...
		default=None,
		type="int",
		help="Iteration of bootstrap to start on [Default:%default]",
	)
	rep_options.add_option(
		"--boot_end",
		dest="boot_end",
		default=None,
		type="int",
		help="Iteration of bootstrap to end on [Default:%default]",
	)
	rep_options.add_option(
		"--identical_crosses",
	...
	# arrange data
	for bs_iter in range(options.boot_start, options.boot_end+1):
		rep_dir = f"{options.out_dir}/bs{bs_iter}"
		...
		for di in range(num_data):
			...
			if not os.path.isdir(rep_data_dir):
				make_rep_data(
					data_dirs[di], rep_data_dir, bs_iter, options.identical_crosses
				)
	...
	for bs_iter in range(options.boot_start, options.boot_end+1):
		...
		if options.restart and not options.checkpoint and os.path.isdir(train_dir):
			...
			if options.transfer:
				pretrained_model = (
					f"{options.transfer}/bs{bs_iter}/train/model_best.pth"
				)
			else:
...
def make_rep_data(data_dir, rep_data_dir, bs_iter, identical_crosses):
	# read data parameters
	...
	np.random.seed(bs_iter)
	# No test fold
	#test_fold = fi 

	# bootstrap training data
	bs_folds = np.random.choice(np.arange(num_folds), size=num_folds, replace=True)
	n_val_folds = int(np.math.ceil(num_folds*(1/8)))
	train_bs_folds = bs_folds[:-n_val_folds]
	valid_bs_folds = bs_folds[-n_val_folds:]

	# Get dictionary representations of training and validation folds
	train_dicti = {}
	for fold in train_bs_folds:
		if fold not in train_dicti:
			train_dicti[fold] = 0
		train_dicti[fold] = train_dicti[fold] + 1
	valid_dicti = {}
	for fold in valid_bs_folds:
		if fold not in valid_dicti:
			valid_dicti[fold] = 0
		valid_dicti[fold] = valid_dicti[fold] + 1
	...
	# dump data stats
	data_stats["test_seqs"] = 0
	data_stats["valid_seqs"] = sum([fold_seqs[tf] for tf in valid_bs_folds])
	data_stats["train_seqs"] = sum([fold_seqs[tf] for tf in train_bs_folds])
	...
	try:
		...
		for line in open("%s/sequences.bed" % data_dir):
			...
			if sfi in train_dicti and sfi in valid_dicti:
				a[-1] = 'train' + str(train_dicti[sfi]) + 'valid' + str(valid_dicti[sfi])
			elif sfi in train_dicti:
				a[-1] = 'train' + str(train_dicti[sfi])
			elif sfi in valid_dicti:
				a[-1] = 'valid' + str(valid_dicti[sfi])
			else:
				a[-1] = "none"
...
def make_rep_data_orig(data_dir, rep_data_dir, fi, ci, identical_crosses):
	# read data parameters
