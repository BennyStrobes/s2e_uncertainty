...
for ii, tissue_name in enumerate(ordered_tissues):
	t.append(open(organized_borzoi_gtex_predictions + tissue_name + '_random_rc_borzoi_estimates_w_uncertainty.txt','w'))
# Print headers
for ii, tissue_name in enumerate(ordered_tissues):
...
for parallel_batch_name in parallel_batch_names:
	...
	for jj, bootstrap_iter in enumerate(bootstraps):
		files.append(open(borzoi_gtex_predictions + 'bs' + str(bootstrap_iter) + '_PIP_0.9_borzoi_pred_eqtl_effects_random_rc_borzoi_sed_results.txt'))
