...
def extract_list_of_trait_names(trait_names_file, chrom_num):
	...
	return np.unique(np.asarray(arr)), np.asarray(genes)
...
def load_in_variant_to_gene_links(organized_pred_results_file, chrom_num, borzoi_effect_thresh):
	f = open(organized_pred_results_file)
	variant_to_links = {}
	head_count = 0
	for line in f:
		line = line.rstrip()
		...
		variant_id = data[1]
		gene_id = data[6]
		borzoi_effect = float(data[-1])

		if np.isnan(borzoi_effect):
			continue

		if np.abs(borzoi_effect) < borzoi_effect_thresh:
			continue

		if variant_id not in variant_to_links:
			variant_to_links[variant_id] = []

		variant_to_links[variant_id].append((gene_id, borzoi_effect))
...
trait_names, burden_genes = extract_list_of_trait_names(burden_genes_summary_file, chrom_num)
...
# Load in variant to gene links for burden genes
variant_to_gene_links = load_in_variant_to_gene_links(organized_pred_results_file, chrom_num, borzoi_effect_thresh)


# Gene name to predicted expression 
gene_name_to_predicted_expression = {}
for gene_name in burden_genes:
	gene_name_to_predicted_expression[gene_name] = np.zeros(len(genotype_sample_indexer))
...
for var in bfile:
	...
	dosage = var.alt_dosage[genotype_sample_indexer]

	for tupler in variant_to_gene_links[new_var_id]:
		tupler_gene = tupler[0]
		tupler_effect = tupler[1]
		gene_name_to_predicted_expression[tupler_gene] = gene_name_to_predicted_expression[tupler_gene] + (dosage*tupler_effect)


bfile.close()


t = open(burden_output_file,'w')
...
for line in f:
	...
	if head_count == 0:
		head_count = head_count + 1
		t.write(line + '\t' + 'non_coding_burden_effect\tnon_coding_burden_effect_se\n')
		continue
	if data[3] != 'chr' + chrom_num:
		continue

	# THis is a disease-gene pair
	trait_name = data[0]
	gene_name = data[1]

	pred_expr = gene_name_to_predicted_expression[gene_name]
	trait_val = trait_to_pheno_vec[trait_name]

	if np.var(pred_expr) == 0.0:
		continue
	else:
		valid_indices = np.isnan(trait_val) == False
		standardized_trait = (trait_val[valid_indices] - np.mean(trait_val[valid_indices]))/np.std(trait_val[valid_indices])
		model = sm.OLS(standardized_trait, sm.add_constant(pred_expr[valid_indices])).fit()
		t.write(line + '\t' + str(model.params[1]) + '\t' + str(model.bse[1]) + '\n')
