...
def create_mapping_from_gene_id_to_expression_vector(expr_file, expression_indices):
	dicti = {}
	head_count = 0
	f = open(expr_file)
	for line in f:
		...
		if head_count == 0:
			head_count = head_count + 1
			expr_sample_names = np.asarray(data[4:])[expression_indices]
			continue
		...
		if gene_id in dicti:
			...
			pdb.set_trace()
		dicti[gene_id] = expr[expression_indices]
	f.close()
...
def generate_input_data_object_for_bayesian_inf(gene_id_to_est_borzoi_effects, gene_id_to_variant_gene_anno, genotype_sample_indices, gene_id_to_expression_vector, genotype_stem, standardize_geno):
	...
	gene_to_training_genotype_info = {}
	...
	for chrom_num in range(1,23):
		...
		for gene_id in [*gene_id_to_est_borzoi_effects]:
			...
			obs_expr = gene_id_to_expression_vector[gene_id]
			expr_mean = np.mean(obs_expr)


			gene_to_data[gene_id] = {}
			gene_to_data[gene_id]['expression'] = obs_expr - expr_mean
			...
			gene_to_training_genotype_info[gene_id] = {}
			gene_to_training_genotype_info[gene_id]['valid_snps'] = valid_snps
			gene_to_training_genotype_info[gene_id]['snp_means'] = snp_means
			gene_to_training_genotype_info[gene_id]['snp_sdevs'] = snp_sdevs
			gene_to_training_genotype_info[gene_id]['nan_impute_vals'] = row_means
			gene_to_training_genotype_info[gene_id]['expr_mean'] = expr_mean



	return gene_to_data, gene_to_training_genotype_info


def predict_test_expression(gene_id_to_est_borzoi_effects, gene_id_to_variant_gene_anno, genotype_sample_indices, gene_id_to_expression_vector, expr_sample_names, genotype_stem, standardize_geno, training_data_genotype_info, gene_name_to_posterior_mean_beta, pred_expr_output_file):
	# Initialize output object
	gene_to_data = {}

	t = gzip.open(pred_expr_output_file,'wt')
	t.write('gene_id\tind_id\tobs_expr\tpred_expr\tpred_expr_no_center\n')
	...
	for chrom_num in range(1,23):
		...
		for gene_id in [*gene_id_to_est_borzoi_effects]:
			...
			geno_mat = (G[cis_genotype_indices,:].compute())[:, genotype_sample_indices]
			nan_rows, nan_cols = np.where(np.isnan(geno_mat))
			geno_mat[nan_rows, nan_cols] = training_data_genotype_info[gene_id]['nan_impute_vals'][nan_rows]


			geno_mat = 2.0 - geno_mat

			snp_means = training_data_genotype_info[gene_id]['snp_means']
			snp_sdevs = training_data_genotype_info[gene_id]['snp_sdevs']
			valid_snps = training_data_genotype_info[gene_id]['valid_snps']
			if standardize_geno == 'True':
				geno_mat = (geno_mat - snp_means[:, None])/snp_sdevs[:, None]
				borzoi_effects_standardized = borzoi_effects_unstandardized*snp_sdevs
			elif standardize_geno == 'False':
			...
			obs_expr = gene_id_to_expression_vector[gene_id]
			GG = np.ascontiguousarray(geno_mat[valid_snps, :])
			pred_expr = np.dot(np.transpose(GG), gene_name_to_posterior_mean_beta[gene_id]) + training_data_genotype_info[gene_id]['expr_mean']
			pred_expr_non_centered = np.dot(np.transpose(GG), gene_name_to_posterior_mean_beta[gene_id])

			for ii, ind_obs_expr in enumerate(obs_expr):
				t.write(gene_id + '\t' + expr_sample_names[ii] + '\t' + str(ind_obs_expr) + '\t' + str(pred_expr[ii]) + '\t' + str(pred_expr_non_centered[ii]) + '\n')
...
@njit(cache=True)
def update_causal_effect_for_single_gene_shell(cur_gene_beta, prior_mean, prior_var, gene_resid_var, resid_expr_vec, genotype_mat):
...
def update_causal_effect_for_single_gene(cur_gene_beta, mu_priors, sig_sq_priors, gene_resid_var, resid_expr_vec, anno_vec, borzoi_vec, genotype_mat):
	prior_means = mu_priors
	per_snp_sig_sq_prior = sig_sq_priors
...
def update_priors(betas, gene_names, inf_input_data_obj, sig_sq_priors, gene_snp_annos):
	...
	if len(all_beta) == 0:
		return sig_sq_priors

	# Ridge prior: beta_j ~ N(0, sig_sq_priors), pooled across all SNPs.
	shape_post = a0 + (len(all_beta)/2.0)
	scale_post = b0 + (0.5*np.sum(np.square(all_beta)))
	return 1.0/np.random.gamma(shape_post, 1.0/scale_post)
...
def run_inference(inf_input_data_obj, burn_in_iters=50, total_iters = 100):
	...
	if burn_in_iters >= total_iters:
		raise ValueError('total_iters must be greater than burn_in_iters')

	posterior_beta_sums = []
	for gene_beta in betas:
		posterior_beta_sums.append(np.zeros_like(gene_beta))
	...
	for itera in range(total_iters):
		...
		# Update priors
		sig_sq_priors = update_priors(betas, gene_names, inf_input_data_obj, sig_sq_priors, gene_snp_annos)
		t2 = time.time()
		...
		if itera >= burn_in_iters:
			...
			for gene_iter, gene_beta in enumerate(betas):
				posterior_beta_sums[gene_iter] = posterior_beta_sums[gene_iter] + gene_beta

	gene_name_to_posterior_mean_beta = {}
	for gene_iter, gene_name in enumerate(gene_names):
		gene_name_to_posterior_mean_beta[gene_name] = posterior_beta_sums[gene_iter]/n_posterior_samples
	return gene_name_to_posterior_mean_beta
...
fold_number = int(sys.argv[9])
n_folds = int(sys.argv[10])
down_sampling_fraction = float(sys.argv[11])

###########################
# Load in data
###########################

# Seed for reliability across folds
np.random.seed(1)
...
# Downsampling
N_orig_samples = len(genotype_sample_indices)
N_downsampled_samples = int(np.floor(N_orig_samples*down_sampling_fraction))
if N_downsampled_samples < 1:
	raise ValueError('down_sampling_fraction leaves zero samples')
if N_downsampled_samples < n_folds:
	raise ValueError('Need at least as many downsampled samples as folds')
if fold_number < 1 or fold_number > n_folds:
	raise ValueError('fold_number must be between 1 and n_folds')

downsampled_expression_indices = np.random.permutation(N_orig_samples)[:N_downsampled_samples]
downsampled_genotype_sample_indices = genotype_sample_indices[downsampled_expression_indices]

# Split into n_folds and get training and test folds
folds = np.array_split(downsampled_expression_indices, n_folds)
test_fold_index = fold_number - 1

expression_testing_indices = np.sort(folds[test_fold_index])
expression_training_indices = np.sort(np.concatenate([folds[i] for i in range(n_folds) if i != test_fold_index]))

genotype_testing_indices = genotype_sample_indices[expression_testing_indices]
genotype_training_indices = genotype_sample_indices[expression_training_indices]
...
gene_id_to_training_expression_vector, training_expr_sample_names = create_mapping_from_gene_id_to_expression_vector(expr_file, expression_training_indices)
gene_id_to_testing_expression_vector, testing_expr_sample_names = create_mapping_from_gene_id_to_expression_vector(expr_file, expression_testing_indices)


# Generate input data Object
inf_input_data_obj, training_data_genotype_info = generate_input_data_object_for_bayesian_inf(gene_id_to_est_borzoi_effects, gene_id_to_variant_gene_anno, genotype_training_indices, gene_id_to_training_expression_vector, genotype_stem, standardize_geno)
del genotype_sample_indices
del gene_id_to_training_expression_vector
del training_expr_sample_names


# Infer genome-wide prior
gene_name_to_posterior_mean_beta = run_inference(inf_input_data_obj, burn_in_iters=800, total_iters = 1000)
del inf_input_data_obj

# Predict expression on held out test data
pred_expr_output_file = output_stem + '_predicted_expression.txt.gz'
predict_test_expression(gene_id_to_est_borzoi_effects, gene_id_to_variant_gene_anno, genotype_testing_indices, gene_id_to_testing_expression_vector, testing_expr_sample_names, genotype_stem, standardize_geno, training_data_genotype_info, gene_name_to_posterior_mean_beta, pred_expr_output_file)
