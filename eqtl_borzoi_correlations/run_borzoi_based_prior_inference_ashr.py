...
def initialize_data(inf_input_data_obj, grid_size, standardize_geno):
	...
	prior_var_pis = np.ones((n_bins, grid_size))/grid_size
	if standardize_geno == 'True':
		prior_var_fixed_grid = np.logspace(-7, 0, num=grid_size)
	elif standardize_geno == 'False':
		prior_var_fixed_grid = np.logspace(-7, 1, num=grid_size)
	...
	return betas, latent_states, mu_priors, prior_var_pis, prior_var_fixed_grid, resid_vars, expression_resids, gene_annos, gene_names,
...
def update_causal_effect_for_single_gene_ashr_shell(cur_gene_beta, cur_gene_latent_state, prior_means, prior_var_pis, prior_var_fixed_grid, anno_vec, gene_resid_var, resid_expr_vec, genotype_mat):
	...
	for snp_iter in np.random.permutation(n_snps):
		...
		beta_hat = geno_resid_dot/sum_sq_geno
		beta_hat_var = gene_resid_var/sum_sq_geno
		for grid_iter in range(n_grid):
			prior_var = prior_var_fixed_grid[grid_iter]
			marginal_var = prior_var + beta_hat_var
			diff = beta_hat - prior_mean
			log_probs[grid_iter] = np.log(prior_var_pis[anno_bin, grid_iter] + 1e-300) - (0.5*np.log(marginal_var)) - (0.5*(diff*diff)/marginal_var)
...
def update_causal_effect_for_single_gene(cur_gene_beta, cur_gene_latent_state, mu_priors, prior_var_pis, prior_var_fixed_grid, gene_resid_var, resid_expr_vec, anno_vec, borzoi_vec, genotype_mat):
	prior_means = (mu_priors[anno_vec])*borzoi_vec
	return update_causal_effect_for_single_gene_ashr_shell(cur_gene_beta, cur_gene_latent_state, prior_means, prior_var_pis, prior_var_fixed_grid, anno_vec, gene_resid_var, resid_expr_vec, genotype_mat)
...
def update_causal_effect_estimates(betas, latent_states, mu_priors, prior_var_pis, prior_var_fixed_grid, resid_vars, expression_resids, gene_snp_annos, gene_names, inf_input_data_obj):
	...
	for gene_iter, gene_name in enumerate(gene_names):
		...
		gene_beta, gene_latent_state, gene_expression_resid = update_causal_effect_for_single_gene(betas[gene_iter], latent_states[gene_iter], mu_priors, prior_var_pis, prior_var_fixed_grid, resid_vars[gene_iter], expression_resids[gene_iter], gene_snp_annos[gene_iter], inf_input_data_obj[gene_name]['borzoi'], inf_input_data_obj[gene_name]['genotype'])
...
def update_priors(betas, latent_states, gene_names, inf_input_data_obj, prior_var_pis, prior_var_fixed_grid, gene_snp_annos):
	# Weak hyperparameters
	tau_sq = 10.0
	dirichlet_alpha = np.ones(len(prior_var_fixed_grid))*1.0
	...
	for gene_iter, gene_name in enumerate(gene_names):
		...
		all_latent_states.append(latent_states[gene_iter])

	all_beta = np.hstack(all_beta)
	all_borzoi = np.hstack(all_borzoi)
	all_anno = np.hstack(all_anno)
	all_latent_states = np.hstack(all_latent_states)

	n_bins = prior_var_pis.shape[0]
	n_grid = len(prior_var_fixed_grid)
	...
	for bin_iter in range(n_bins):
		...
		bin_latent_states = all_latent_states[indices]
		if len(bin_beta) == 0:
			mu_priors[bin_iter] = np.random.normal(loc=0.0, scale=np.sqrt(tau_sq))
			prior_var_pis[bin_iter, :] = np.random.dirichlet(dirichlet_alpha)
			continue

		per_snp_prior_var = prior_var_fixed_grid[bin_latent_states]
		posterior_var = 1.0/((np.sum(np.square(bin_borzoi)/per_snp_prior_var)) + (1.0/tau_sq))
		posterior_mean = posterior_var*(np.sum((bin_borzoi*bin_beta)/per_snp_prior_var))
		...
		prior_var_pis[bin_iter, :] = np.random.dirichlet(dirichlet_alpha + counts)

		signal = np.var(mu_priors[bin_iter]*bin_borzoi)
		avg_prior_var = np.sum(prior_var_pis[bin_iter, :]*prior_var_fixed_grid)
		r_sq = signal/(signal + avg_prior_var)
		r_squares[bin_iter] = r_sq

	return mu_priors, prior_var_pis, r_squares
...
def save_prior_variance_grid(output_stem, prior_var_fixed_grid):
	grid_output_file = output_stem + '_prior_variance_grid.txt'
	t = open(grid_output_file, 'w')
	t.write('grid_index\tprior_variance\n')
...
def create_posterior_param_names(n_bins, n_grid):
	...
	for param_iter in range(n_bins):
		for grid_iter in range(n_grid):
			param_names.append('prior_var_pi_' + str(param_iter) + '_' + str(grid_iter))
	for param_iter in range(n_bins):
		param_names.append('r_square_' + str(param_iter))
	return param_names

def run_inference(inf_input_data_obj, output_stem, standardize_geno, burn_in_iters=50, total_iters = 100, grid_size=8):
	# Initialize model parameters
	betas, latent_states, mu_priors, prior_var_pis, prior_var_fixed_grid, resid_vars, expression_resids, gene_snp_annos, gene_names = initialize_data(inf_input_data_obj, grid_size, standardize_geno)

	save_prior_variance_grid(output_stem, prior_var_fixed_grid)
	...
	for itera in range(total_iters):
		...
		betas, latent_states, expression_resids, resid_vars = update_causal_effect_estimates(betas, latent_states, mu_priors, prior_var_pis, prior_var_fixed_grid, resid_vars, expression_resids, gene_snp_annos, gene_names, inf_input_data_obj)

		# Update priors
		mu_priors, prior_var_pis, r_squares = update_priors(betas, latent_states, gene_names, inf_input_data_obj, prior_var_pis, prior_var_fixed_grid, gene_snp_annos)
		t2 = time.time()

		print('###################', flush=True)
		print('Iteration ' + str(itera), flush=True)
		print(mu_priors, flush=True)
		print(prior_var_pis, flush=True)
		print(r_squares, flush=True)
		print(t2-t1, flush=True)


		if itera >= burn_in_iters:
			posterior_samples.append(np.hstack((mu_priors, np.hstack(prior_var_pis), r_squares)))

	if len(posterior_samples) > 0:
		param_names = create_posterior_param_names(len(mu_priors), prior_var_pis.shape[1])
...
run_inference(inf_input_data_obj, output_stem, standardize_geno, burn_in_iters=3000, total_iters = 3500, grid_size=16)
