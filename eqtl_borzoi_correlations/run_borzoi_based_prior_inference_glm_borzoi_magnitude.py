...
def initialize_data(inf_input_data_obj):
	...
	n_anno = inf_input_data_obj[gene_names[0]]['anno'].shape[1]
	n_genes = len(gene_names)

	# Initialize priors
	mu_priors = 1.0
	gamma_priors = np.zeros(n_anno)
	...
	for gene_name in gene_names:
		...
		sig_sq_priors.append(np.ones(len(borzoi_preds))/300.0)
	...
	return betas, mu_priors, sig_sq_priors, resid_vars, expression_resids, gene_names, gamma_priors
...
def update_causal_effect_for_single_gene(cur_gene_beta, mu_priors, per_snp_sig_sq_prior, gene_resid_var, resid_expr_vec, anno_vec, borzoi_vec, genotype_mat):
	prior_means = (mu_priors)*borzoi_vec
...
def update_causal_effect_estimates(betas, mu_priors, sig_sq_priors, resid_vars, expression_resids, gene_names, inf_input_data_obj, n_coordinate_sweeps=3):
	...
	for gene_iter, gene_name in enumerate(gene_names):
		...
		prior_means = mu_priors*inf_input_data_obj[gene_name]['borzoi']
...
def compute_gamma_objective(all_anno, gamma_priors, expected_sq_resid, lam):
	eta = np.dot(all_anno, gamma_priors)
	objective = 0.5*np.sum(eta + (expected_sq_resid*np.exp(-eta)))
	objective = objective + (0.5*lam*np.sum(np.square(gamma_priors[1:])))
	return objective

def update_priors(beta_means, beta_vars, gene_names, inf_input_data_obj, gamma_priors, tau_sq=100.0, gamma_tau_sq=.01, max_newton_steps=50, max_m_step_iters=5):
	all_beta_mean = []
	all_beta_var = []
	all_borzoi = []
	all_anno = []
	...
	for gene_iter, gene_name in enumerate(gene_names):
		...
		all_anno.append(inf_input_data_obj[gene_name]['anno'])
	...
	all_anno = np.vstack(all_anno)

	lam = 1.0/gamma_tau_sq
	gamma_priors = np.copy(gamma_priors)
	identity = np.eye(all_anno.shape[1])
	mu_priors = 0.0
	for _ in range(max_m_step_iters):
		eta = np.dot(all_anno, gamma_priors)
		sig_sq = np.exp(eta)

		mu_denom = np.sum(np.square(all_borzoi)/sig_sq) + (1.0/tau_sq)
		mu_numer = np.sum((all_borzoi*all_beta_mean)/sig_sq)
		mu_priors = mu_numer/mu_denom

		expected_sq_resid = all_beta_var + np.square(all_beta_mean - (mu_priors*all_borzoi))
		for _ in range(max_newton_steps):
			eta = np.dot(all_anno, gamma_priors)
			inv_sig_sq = np.exp(-eta)
			weighted_resid = expected_sq_resid*inv_sig_sq
			grad = 0.5*np.dot(all_anno.T, (1.0 - weighted_resid))
			grad[1:] = grad[1:] + (lam*gamma_priors[1:])
			hess = 0.5*np.dot(all_anno.T*weighted_resid, all_anno)
			hess[1:, 1:] = hess[1:, 1:] + (lam*identity[1:, 1:])
			step = np.linalg.solve(hess, grad)

			cur_objective = compute_gamma_objective(all_anno, gamma_priors, expected_sq_resid, lam)
			step_size = 1.0
			while step_size > 1e-8:
				proposed_gamma = gamma_priors - (step_size*step)
				proposed_objective = compute_gamma_objective(all_anno, proposed_gamma, expected_sq_resid, lam)
				if proposed_objective <= cur_objective:
					gamma_priors = proposed_gamma
					break
				step_size = step_size/2.0

			if step_size <= 1e-8 or np.max(np.abs(step_size*step)) < 1e-6:
				break

		eta = np.dot(all_anno, gamma_priors)
		sig_sq = np.exp(eta)
	print(np.sort(sig_sq))
	mu_denom = np.sum(np.square(all_borzoi)/sig_sq) + (1.0/tau_sq)
	mu_numer = np.sum((all_borzoi*all_beta_mean)/sig_sq)
	mu_priors = mu_numer/mu_denom
	...
	signal = np.var(mu_priors*all_borzoi)
	avg_prior_var = np.mean(sig_sq)
	r_squares = np.asarray([signal/(signal + avg_prior_var)])

	return mu_priors, sig_sq_priors, gamma_priors, r_squares
...
def create_posterior_param_names(n_gamma):
	param_names = []
	param_names.append('mu_prior')
	for param_iter in range(n_gamma):
		param_names.append('gamma_prior_' + str(param_iter))
	param_names.append('r_square')
	return param_names

def run_inference(inf_input_data_obj, output_stem, burn_in_iters=50, total_iters = 100):
	# Initialize model parameters
	betas, mu_priors, sig_sq_priors, resid_vars, expression_resids, gene_names, gamma_priors = initialize_data(inf_input_data_obj)
	...
	for itera in range(total_iters):
		...
		betas, posterior_beta_vars, expression_resids, resid_vars = update_causal_effect_estimates(betas, mu_priors, sig_sq_priors, resid_vars, expression_resids, gene_names, inf_input_data_obj)

		print('update priors')
		# M-step: update mu and gamma using posterior second moments.
		mu_priors, sig_sq_priors, gamma_priors, r_squares = update_priors(betas, posterior_beta_vars, gene_names, inf_input_data_obj, gamma_priors)
		t2 = time.time()

		print('###################', flush=True)
		print('Iteration ' + str(itera), flush=True)
		print(mu_priors, flush=True)
		print(gamma_priors, flush=True)
		print(r_squares, flush=True)
		print(t2-t1, flush=True)

		if itera >= burn_in_iters:
			posterior_samples.append(np.hstack((np.asarray([mu_priors]), gamma_priors, r_squares)))

	if len(posterior_samples) > 0:
		param_names = create_posterior_param_names(len(gamma_priors))
