...
DEFAULT_GRID_ABS_DELTA_KNOTS = np.asarray([0.025, 0.1, 0.25])
DEFAULT_GRID_KNOTS = np.square(DEFAULT_GRID_ABS_DELTA_KNOTS)


def create_continuous_piecewise_linear_squared_basis(delta, knots=DEFAULT_GRID_KNOTS):
...
def generate_gene_ld_means(gene_id_to_est_borzoi_effects,gene_id_to_est_borzoi_effects_unstandardized, gene_id_to_est_eqtl_effects, onek_genomes_plink_filestem, genotype_sample_indices, variant_id_to_genotype_sdev, anno_method):
	...
	for chrom_num in range(1,23):
		...
		for gene_id in [*gene_id_to_est_borzoi_effects]:
			...
			allelic_grid_basis = create_continuous_piecewise_linear_squared_basis(borzoi_effects_unstandardized)
			gene_to_ld_means[gene_id]['ld_score_allelic_grid_squared_basis'] = np.dot(sq_LD, np.square(borzoi_sdevs)[:, None]*allelic_grid_basis)
...
def fit_grid_ldscore_model(gene_id_to_ld_means, calibration_coef, calibration_coef_var=0.0, calibration_n_bootstraps=0, knots=DEFAULT_GRID_KNOTS, intercept=False):
	gene_regression_data = []
	all_response = []
	all_uncorrected_response = []
	all_calibration_correction = []
	...
	for gene_name in [*gene_id_to_ld_means]:
		...
		valid_rows = np.isfinite(gene_response)
		if intercept:
			gene_design_mat = np.hstack((
				np.ones((len(gene_response), 1)),
				gene_info['ld_score_allelic_grid_squared_basis']
			))
		else:
			gene_design_mat = gene_info['ld_score_allelic_grid_squared_basis']
		...
		all_uncorrected_response.append(gene_uncorrected_response[valid_rows])
		all_calibration_correction.append(gene_calibration_correction[valid_rows])
		all_delta.append(gene_info['borzoi_effects_unstandardized'])
		design_mats.append(gene_design_mat[valid_rows, :])

	if len(gene_regression_data) == 0:
		print('assumption eroror: no valid rows for ldscore grid regression')
		pdb.set_trace()

	all_response = np.hstack(all_response)
	all_uncorrected_response = np.hstack(all_uncorrected_response)
	all_calibration_correction = np.hstack(all_calibration_correction)
	all_delta = np.hstack(all_delta)
	design_mat = np.vstack(design_mats)
	...
	if intercept:
		...
		ldscore_intercept = 0.0
		grid_coefs = np.copy(coef)
	...
	for gene_name in [*gene_id_to_ld_means]:
		gene_info = gene_id_to_ld_means[gene_name]
		allelic_grid_basis = create_continuous_piecewise_linear_squared_basis(gene_info['borzoi_effects_unstandardized'], knots=knots)
		per_allele_var = np.dot(allelic_grid_basis, grid_coefs)
		gene_info['grid_allelic_variance_pred'] = per_allele_var
		gene_info['grid_standardized_variance_pred'] = np.square(gene_info['borzoi_variant_sdevs'])*per_allele_var
		gene_info['grid_variance_pred_from_ld'] = ldscore_intercept + np.dot(gene_info['ld_score_allelic_grid_squared_basis'], grid_coefs)
		gene_info['eqtl_residual_sq_minus_se_sq_uncorrected'] = np.square(gene_info['eQTL_effect_sizes'] - (calibration_coef*gene_info['ld_means'])) - np.square(gene_info['eQTL_effect_ses'])
		gene_info['calibration_uncertainty_correction'] = calibration_coef_var*np.square(gene_info['ld_means'])
		gene_info['eqtl_residual_sq_minus_se_sq'] = gene_info['eqtl_residual_sq_minus_se_sq_uncorrected'] - gene_info['calibration_uncertainty_correction']

	grid_basis = create_continuous_piecewise_linear_squared_basis(all_delta, knots=knots)
	all_pred_allelic_var = np.dot(grid_basis, grid_coefs)

	fit_info = {}
	fit_info['calibration_coef'] = calibration_coef
	fit_info['model_effect_scale'] = 'allelic_grid_squared'
	fit_info['calibration_coef_var'] = calibration_coef_var
	fit_info['calibration_coef_se'] = np.sqrt(calibration_coef_var)
	fit_info['calibration_n_bootstraps'] = calibration_n_bootstraps
	fit_info['ldscore_intercept'] = ldscore_intercept
	fit_info['grid_knots'] = knots
	fit_info['grid_coefs'] = grid_coefs
	fit_info['optimization_success'] = True
	fit_info['optimization_status'] = 0
	fit_info['optimization_message'] = 'ordinary_least_squares'
	fit_info['optimization_objective'] = 0.5*np.sum(np.square(all_response - fitted_y))
	...
	fit_info['mean_calibration_uncertainty_correction'] = np.mean(all_calibration_correction)
	fit_info['mean_fitted_response'] = np.mean(fitted_y)
	fit_info['mean_predicted_allelic_prior_var'] = np.mean(all_pred_allelic_var)
	fit_info['median_predicted_allelic_prior_var'] = np.median(all_pred_allelic_var)
	fit_info['min_predicted_allelic_prior_var'] = np.min(all_pred_allelic_var)
	fit_info['max_predicted_allelic_prior_var'] = np.max(all_pred_allelic_var)
	return gene_id_to_ld_means, fit_info


def save_grid_ldscore_fit(output_stem, fit_info):
	summary_output_file = output_stem + '_ldscore_grid_squared_summary.txt'
	with open(summary_output_file, 'w') as t:
		t.write('field\tvalue\n')
		t.write('calibration_coef\t' + str(fit_info['calibration_coef']) + '\n')
		t.write('model_effect_scale\t' + str(fit_info['model_effect_scale']) + '\n')
		...
		t.write('calibration_n_bootstraps\t' + str(fit_info['calibration_n_bootstraps']) + '\n')
		t.write('ldscore_intercept\t' + str(fit_info['ldscore_intercept']) + '\n')
		t.write('grid_knots\t' + ','.join(fit_info['grid_knots'].astype(str)) + '\n')
		t.write('grid_abs_delta_knots\t' + ','.join(np.sqrt(fit_info['grid_knots']).astype(str)) + '\n')
		t.write('optimization_success\t' + str(fit_info['optimization_success']) + '\n')
		t.write('optimization_status\t' + str(fit_info['optimization_status']) + '\n')
		t.write('optimization_message\t' + str(fit_info['optimization_message']).replace('\n', ' ') + '\n')
		t.write('optimization_objective\t' + str(fit_info['optimization_objective']) + '\n')
		...
		t.write('mean_calibration_uncertainty_correction\t' + str(fit_info['mean_calibration_uncertainty_correction']) + '\n')
		t.write('mean_fitted_response\t' + str(fit_info['mean_fitted_response']) + '\n')
		t.write('mean_predicted_allelic_prior_var\t' + str(fit_info['mean_predicted_allelic_prior_var']) + '\n')
		t.write('median_predicted_allelic_prior_var\t' + str(fit_info['median_predicted_allelic_prior_var']) + '\n')
		t.write('min_predicted_allelic_prior_var\t' + str(fit_info['min_predicted_allelic_prior_var']) + '\n')
		t.write('max_predicted_allelic_prior_var\t' + str(fit_info['max_predicted_allelic_prior_var']) + '\n')

	param_output_file = output_stem + '_ldscore_grid_squared_params.txt'
	with open(param_output_file, 'w') as t:
		t.write('parameter\tindex\tvalue\n')
		t.write('a_prior\t0\t' + str(fit_info['calibration_coef']) + '\n')
		t.write('a_prior_var\t0\t' + str(fit_info['calibration_coef_var']) + '\n')
		t.write('a_prior_se\t0\t' + str(fit_info['calibration_coef_se']) + '\n')
		t.write('ldscore_intercept\t0\t' + str(fit_info['ldscore_intercept']) + '\n')
		for coef_iter, coef_val in enumerate(fit_info['grid_coefs']):
			t.write('grid_coef\t' + str(coef_iter) + '\t' + str(coef_val) + '\n')

	curve_output_file = output_stem + '_ldscore_grid_squared_curve.txt'
	max_abs_delta = np.sqrt(np.max(fit_info['grid_knots']))*2.0
	grid_abs_delta = np.linspace(0.0, max_abs_delta, 200)
	grid_basis = create_continuous_piecewise_linear_squared_basis(grid_abs_delta, knots=fit_info['grid_knots'])
	grid_var = np.dot(grid_basis, fit_info['grid_coefs'])
	with open(curve_output_file, 'w') as t:
		t.write('abs_delta_allelic\tdelta_allelic_squared\tpred_allelic_variance\n')
		for x_val, var_val in zip(grid_abs_delta, grid_var):
			t.write(str(x_val) + '\t' + str(x_val*x_val) + '\t' + str(var_val) + '\n')

	fit_pickle_file = output_stem + '_ldscore_grid_squared_fit.pkl'
	with open(fit_pickle_file, 'wb') as f:
		pickle.dump(fit_info, f)
...
calibration_coef_var = 0

gene_id_to_ld_means, grid_fit_info = fit_grid_ldscore_model(gene_id_to_ld_means, calibration_coef, calibration_coef_var=calibration_coef_var, calibration_n_bootstraps=calibration_n_bootstraps)
print('model_effect_scale: ' + str(grid_fit_info['model_effect_scale']), flush=True)
print('ldscore_intercept: ' + str(grid_fit_info['ldscore_intercept']), flush=True)
print('grid_coefs: ' + ','.join(grid_fit_info['grid_coefs'].astype(str)), flush=True)
'''
with open(pickle_output_file, 'wb') as f:
	pickle.dump(gene_id_to_ld_means, f)
'''
save_grid_ldscore_fit(borzoi_based_prior_output_stem, grid_fit_info)
