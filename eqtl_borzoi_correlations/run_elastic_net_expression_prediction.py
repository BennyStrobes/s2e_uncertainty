...
def generate_input_data_object_for_bayesian_inf(gene_id_to_est_borzoi_effects, gene_id_to_variant_gene_anno, genotype_sample_indices, gene_id_to_expression_vector, genotype_stem, standardize_geno):
	...
	for chrom_num in range(1,23):
		...
		for gene_id in [*gene_id_to_est_borzoi_effects]:
			...
			expr_vec = obs_expr - expr_mean
			geno_mat = np.transpose(np.ascontiguousarray(geno_mat[valid_snps, :]))
			...
			try:
				...
				model.fit(geno_mat, expr_vec)
				gene_beta = model.coef_
			except Exception as e:
				print('ElasticNetCV failed for ' + gene_id + ': ' + str(e), flush=True)
				gene_beta = np.zeros(geno_mat.shape[1])


			if gene_id in gene_to_betas:
				print('errorr')
				pdb.set_trace()
			gene_to_betas[gene_id] = gene_beta
	...
	return gene_to_betas, gene_to_training_genotype_info
...
gene_name_to_posterior_mean_beta, training_data_genotype_info = generate_input_data_object_for_bayesian_inf(gene_id_to_est_borzoi_effects, gene_id_to_variant_gene_anno, genotype_training_indices, gene_id_to_training_expression_vector, genotype_stem, standardize_geno)
