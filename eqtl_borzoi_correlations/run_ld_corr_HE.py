...
def generate_gene_ld_means(gene_id_to_est_borzoi_effects,gene_id_to_est_borzoi_effects_unstandardized, gene_id_to_est_eqtl_effects, gene_id_to_variant_gene_anno, onek_genomes_plink_filestem, anno_names, genotype_sample_indices):
	...
	for chrom_num in range(21,23):
		...
		for gene_id in [*gene_id_to_est_borzoi_effects]:
			...
			eqtl_LD = LD[:, observed_eqtl_indices]
			# B. on borzoi end
			...
			sampling_covariance = (eqtl_effect_ses[:, None]*eqtl_LD*eqtl_effect_ses[None, :])[upper_tri_indices]
			annotation_covariances = np.asarray([
				((LD*variant_anno[:, anno_iter][None, :]) @ LD.T)[upper_tri_indices]
			...
			calibration_x = ld_means[valid_calibration_rows, :]
			calibration_y = eqtl_effects[valid_calibration_rows]
			calibration_xt_x = calibration_x.T @ calibration_x
			calibration_xt_y = calibration_x.T @ calibration_y

			gene_to_ld_means[gene_id] = {}
			gene_to_ld_means[gene_id]['he_xt_x'] = he_xt_x
			gene_to_ld_means[gene_id]['he_xt_y'] = he_xt_y
			gene_to_ld_means[gene_id]['calibration_xt_x'] = calibration_xt_x
			gene_to_ld_means[gene_id]['calibration_xt_y'] = calibration_xt_y
