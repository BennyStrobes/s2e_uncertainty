...
output_file_std = borzoi_output_dir + target_tissue + '_' + target_sample + '_borzoi_effects_compete_version.txt.gz'
t2 = gzip.open(output_file_std,'wt')
t2.write('gene\tvariant\tchr\tsnp_pos\ta0\ta1\tborzoi_effect_size\n')
...
for snp_chrom, snp_pos, snp_ids, gene_ids, borzoi_effects in stream_borzoi_results(borzoi_results_dir, borzoi_target_index):
	for chrom, pos, variant_id, gene_id, borzoi_effect_size in zip(snp_chrom, snp_pos, snp_ids, gene_ids, borzoi_effects):
		...
		t2.write(gene_name + '\t' + variant_id + '\t' + chrom_string + '\t' + str(pos) + '\t' + var_info[2] + '\t' + var_info[3] + '\t' + str(borzoi_effect_size) + '\n')
