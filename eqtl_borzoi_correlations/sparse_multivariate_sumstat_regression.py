...
class SPARSE_SLDSC_ARD_SOME_FIXED_MV_UPDATES(object):
	def __init__(self, L=10, regularization_param=1e-20, max_iter=10000, convergence_thresh=1e-20, learn_fixed_variance=False, estimate_prior_variance=True, ard_a_prior=1e-16, ard_b_prior=1e-16, nonneg=False, nonneg_int=93):
	...
	def fit(self, tau, tau_cov):
		...
		for itera in range(self.max_iter):
			...
			self.update_causal_effects(self.component_variances)


			#if self.estimate_prior_variance and itera > 10000:
			if self.estimate_prior_variance and itera > 10:
	...
	def update_causal_effects(self, component_variances):
		...
		coef_prior_precisions = 1.0/self.component_variances
	...
	def update_component_variances(self):
		...
		for kk in range(len(self.component_variances)):
			component_variance_a_term = ((expected_a_terms[kk])/2.0) + 1e-30
