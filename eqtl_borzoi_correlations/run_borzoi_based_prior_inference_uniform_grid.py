...
def generate_input_data_object_for_bayesian_inf(gene_id_to_est_borzoi_effects, gene_id_to_variant_gene_anno, genotype_sample_indices, gene_id_to_expression_vector, genotype_stem):
	...
	for chrom_num in range(8,23):
		...
		for gene_id in [*gene_id_to_est_borzoi_effects]:
			...
			gene_to_data[gene_id]['borzoi'] = borzoi_effects_unstandardized[valid_snps]
			gene_to_data[gene_id]['genotype'] = np.ascontiguousarray(geno_mat[valid_snps, :])
			gene_to_data[gene_id]['snp_sdevs'] = snp_sdevs[valid_snps]
...
def initialize_data(inf_input_data_obj, grid_size):
	...
	# Grid 1
	effect_size_grid = np.array([
		-1.5, -0.6, -0.2, -0.06, -0.02, -0.003, 0.0,
		 0.003, 0.02, 0.06, 0.2, 0.6, 1.5
	])

	# Grid 2
	effect_size_grid = np.array([
		-1.0, -0.4, -0.15, -0.05, -0.015, -0.0025, 0.0,
		 0.0025, 0.015, 0.05, 0.15, 0.4, 1.0
	])


	effect_size_grid_lower, effect_size_grid_upper = create_uniform_effect_size_grid_bounds(effect_size_grid)
	...
	return betas, latent_states, effect_size_grid, effect_size_grid_lower, effect_size_grid_upper, effect_size_grid_pis, resid_vars, expression_resids, gene_annos, gene_names,


def create_uniform_effect_size_grid_bounds(effect_size_grid):
	n_grid = len(effect_size_grid)
	if n_grid < 2:
		print('assumption error: effect size grid too small')
		pdb.set_trace()
	midpoints = (effect_size_grid[1:] + effect_size_grid[:-1])/2.0
	effect_size_grid_lower = np.zeros(n_grid)
	effect_size_grid_upper = np.zeros(n_grid)
	effect_size_grid_lower[1:] = midpoints
	effect_size_grid_upper[:-1] = midpoints
	effect_size_grid_lower[0] = effect_size_grid[0] - (midpoints[0] - effect_size_grid[0])
	effect_size_grid_upper[-1] = effect_size_grid[-1] + (effect_size_grid[-1] - midpoints[-1])
	return effect_size_grid_lower, effect_size_grid_upper


def sample_categorical_from_log_probs_python(log_probs):
	max_log_prob = np.max(log_probs)
	unnorm_probs = np.exp(log_probs - max_log_prob)
	unnorm_probs = unnorm_probs/np.sum(unnorm_probs)
	return np.random.choice(np.arange(len(unnorm_probs)), p=unnorm_probs)


@njit(cache=True)
def normal_cdf_numba(x):
	return 0.5*(1.0 + math.erf(x/math.sqrt(2.0)))


@njit(cache=True)
def normal_inv_cdf_numba(p):
	if p <= 0.0:
		return -1e100
	if p >= 1.0:
		return 1e100
...
@njit(cache=True)
def sample_truncated_normal_numba(mean, sd, lower_bound, upper_bound):
	if sd <= 0.0:
		if mean < lower_bound:
			return lower_bound
		if mean > upper_bound:
	...
	lower_cdf = normal_cdf_numba((lower_bound - mean)/sd)
	upper_cdf = normal_cdf_numba((upper_bound - mean)/sd)
	...
	u = np.random.uniform(lower_cdf, upper_cdf)
	if u <= 1e-12:
		u = 1e-12
	elif u >= (1.0 - 1e-12):
		u = 1.0 - 1e-12
	return mean + (sd*normal_inv_cdf_numba(u))
...
@njit(cache=True)
def update_causal_effect_for_single_gene_uniform_grid_shell(cur_gene_beta, cur_gene_latent_state, fixed_effect_size_grid, effect_size_grid_lower, effect_size_grid_upper, effect_size_grid_pis, gene_resid_var, resid_expr_vec, anno_vec, genotype_mat):
	n_snps = len(cur_gene_beta)
	n_grid = len(fixed_effect_size_grid)
	n_samples = len(resid_expr_vec)
	log_probs = np.zeros(n_grid)
	...
	for snp_iter in np.random.permutation(n_snps):
		...
		for sample_iter in range(n_samples):
			...
			sum_sq_geno = sum_sq_geno + (geno_val*geno_val)
			geno_resid_dot = geno_resid_dot + (geno_val*resid_expr_vec[sample_iter])
		cond_var = gene_resid_var/sum_sq_geno
		cond_mean = geno_resid_dot/sum_sq_geno
		cond_sd = math.sqrt(cond_var)
		anno_bin = anno_vec[snp_iter]

		for grid_iter in range(n_grid):
			lower_bound = effect_size_grid_lower[grid_iter]
			upper_bound = effect_size_grid_upper[grid_iter]
			interval_width = upper_bound - lower_bound
			lower_cdf = normal_cdf_numba((lower_bound - cond_mean)/cond_sd)
			upper_cdf = normal_cdf_numba((upper_bound - cond_mean)/cond_sd)
			interval_mass = upper_cdf - lower_cdf
			if interval_mass < 1e-300:
				interval_mass = 1e-300
			log_probs[grid_iter] = math.log(effect_size_grid_pis[anno_bin, grid_iter] + 1e-300) - math.log(interval_width) + math.log(interval_mass)

		new_latent_state = sample_categorical_from_log_probs(log_probs)
		new_beta = sample_truncated_normal_numba(cond_mean, cond_sd, effect_size_grid_lower[new_latent_state], effect_size_grid_upper[new_latent_state])
...
def update_causal_effect_for_single_gene(cur_gene_beta, cur_gene_latent_state, fixed_effect_size_grid, effect_size_grid_lower, effect_size_grid_upper, effect_size_grid_pis, gene_resid_var, resid_expr_vec, anno_vec, genotype_mat):
	return update_causal_effect_for_single_gene_uniform_grid_shell(cur_gene_beta, cur_gene_latent_state, fixed_effect_size_grid, effect_size_grid_lower, effect_size_grid_upper, effect_size_grid_pis, gene_resid_var, resid_expr_vec, anno_vec, genotype_mat)
...
def update_causal_effect_estimates(betas, latent_states, fixed_effect_size_grid, effect_size_grid_lower, effect_size_grid_upper, effect_size_grid_pis, resid_vars, expression_resids, gene_snp_annos, gene_names, inf_input_data_obj):
	...
	for gene_iter, gene_name in enumerate(gene_names):
		...
		gene_beta, gene_latent_state, gene_expression_resid = update_causal_effect_for_single_gene(betas[gene_iter], latent_states[gene_iter], fixed_effect_size_grid, effect_size_grid_lower, effect_size_grid_upper, effect_size_grid_pis, resid_vars[gene_iter], expression_resids[gene_iter], gene_snp_annos[gene_iter], inf_input_data_obj[gene_name]['genotype'])
...
def update_priors(latent_states, effect_size_grid_pis, fixed_effect_size_grid, gene_snp_annos):
	dirichlet_alpha = np.ones(len(fixed_effect_size_grid))*0.1
...
def save_prior_effect_size_grid(output_stem, prior_var_fixed_grid, effect_size_grid_lower, effect_size_grid_upper):
	grid_output_file = output_stem + '_fixed_effect_size_grid.txt'
	t = open(grid_output_file, 'w')
	t.write('grid_index\teffect_size\tlower_bound\tupper_bound\n')
	for grid_iter, grid_val in enumerate(prior_var_fixed_grid):
		t.write(str(grid_iter) + '\t' + str(grid_val) + '\t' + str(effect_size_grid_lower[grid_iter]) + '\t' + str(effect_size_grid_upper[grid_iter]) + '\n')
	t.close()
...
def run_inference(inf_input_data_obj, output_stem, burn_in_iters=50, total_iters = 100, grid_size=13):
	...
	betas, latent_states, fixed_effect_size_grid, effect_size_grid_lower, effect_size_grid_upper, effect_size_grid_pis, resid_vars, expression_resids, gene_snp_annos, gene_names = initialize_data(inf_input_data_obj, grid_size)

	save_prior_effect_size_grid(output_stem, fixed_effect_size_grid, effect_size_grid_lower, effect_size_grid_upper)
	...
	for itera in range(total_iters):
		...
		betas, latent_states, expression_resids, resid_vars = update_causal_effect_estimates(betas, latent_states, fixed_effect_size_grid, effect_size_grid_lower, effect_size_grid_upper, effect_size_grid_pis, resid_vars, expression_resids, gene_snp_annos, gene_names, inf_input_data_obj)
...
if standardize_geno == 'True':
	print('assumption erororro: standardized geno not yet implemented')
	pdb.set_trace()
...
run_inference(inf_input_data_obj, output_stem, burn_in_iters=4000, total_iters = 4500, grid_size=13)
